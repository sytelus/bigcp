...
**Distinct-drive local NTFS relative-create specialization (ADR 0048):** when both endpoints are local NTFS, their physical extents do not intersect, and the static transport is standard, each directory-affine worker caches one verified destination-parent handle. Plain-small final names are opened with `NtCreateFile`, that handle as `RootDirectory`, and only the final UTF-16 component, avoiding repeated absolute-parent resolution. The parent denies delete sharing to pin that directory namespace; its kind and captured identity are checked once per worker/directory. An unavailable capability falls back to the ordinary absolute-path constructor. `FILE_CREATE` retains `CREATE_NEW` collision safety, while replacements retain same-handle snapshot validation before truncation. Inline, transactional, sparse, large, same-spindle, ReFS, FAT/exFAT, UNC, and WSL paths do not carry this capability. Its correctness and isolation are confined-test proven; the speedup remains pending the approved multi-drive NTFS benchmark in `BENCHMARKS.md`.
...
select_profile(side):                       # once per volume at startup; never changes mid-run
  ...
  else if mem is set: reserve two pipelined coordinator chunks (ADR 0055); cap workers by remaining / large_threshold
...
| I9 | Copy-data buffers, journal replay, and worker queues are bounded; directory-join metadata is the documented pre-1.0 exception | bounded channels, one-record journal lookahead, fallible small/pipeline/burst reservation, exact standard `chunk + workers×threshold` and redirector `2×chunk + workers×max(threshold, 2×chunk)` accounting, serialized same-spindle burst cap; single-directory map gap tracked in §5.6/LIMITATIONS.md |
...
| Streaming I/O | synchronous buffered `ReadFile`/`WriteFile` in profile-sized chunks: sequential on standard, fixed two-buffer source/destination overlap on generic redirectors and WSL, phased bursts on same-spindle; WSL destination handles add `FILE_FLAG_SEQUENTIAL_SCAN` as a cache hint (no `FILE_FLAG_NO_BUFFERING`, completion-port APIs, or alignment rules — §5.9, ADRs 0028/0036/0045/0046) |
