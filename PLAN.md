...
3. Disk: open `\\.\PhysicalDriveN` (access 0) → `IOCTL_STORAGE_QUERY_PROPERTY`:
   ...
   - (Vendor/model strings, hotplug-policy inference, and device write-cache readouts were specified in an earlier revision and deleted: they informed cosmetic Devices-tab lines and a hint, not any decision the engine makes. The queries above — seek penalty, bus, MTL, sector alignment — are exactly the set a profile decision consumes.)
...
**Performance hypotheses register.** Numbers this plan quotes for mechanisms not yet measured on target-class hardware are *hypotheses*, tracked in BENCHMARKS.md and validated (or corrected, with the design revisited) when their layer lands: **H2** uniform temp+rename (two disposition calls + rename per small file, §4.3) costs <2 % on the small-file workload — if it measures worse, the answer is optimizing the close path, never reintroducing direct final-name writes; **H3** always-on xxh3 <2 % of one core per GB/s (§5.11); **H4** post-read source revalidation <2 % on the small-file workload — else its default scope narrows per §4.8; **H5** `FlushFileBuffers` honored across the tested drive matrix (§7.5's capability reporting exists precisely because this is not universal). (**H1**, the deferred-close finalizer-pool speedup, was retired together with that design — the current worker-owned close path is the design, kept because nothing measured demands otherwise.) The *mechanisms* stand on correctness grounds; only the *numbers* await measurement.
...
What remains before a 1.0 claim is **verification and evidence only — no product code is missing** (open-item registry: `PLAN_DEVIATIONS.md`; disposition history: `docs/REVIEW_2026-07-29.md`):
...
- **Elevated graceful-VHDX ReFS matrix (§12.5)** — pins ReFS publication (`FileRenameInfoEx`); a capability-gated legacy rename fallback is added there if the probe demands it.
