...
| I/O strategy | **One product engine, two completion paths and four isolated transports**: local standard keeps request-at-a-time buffered I/O for small files while its large streams overlap one read with one write through the redirectors' bounded pipeline — locally with a third buffer and a dedicated in-flight-hash stage so the always-on digest stays off both hot legs (ADR 0055 — half-duplex alternation measured ~25% behind on distinct NVMe; ADR 0057 — hash+write serialization measured at the alternation ceiling on same-SSD); generic redirectors overlap one read with one write through two bounded buffers; WSL reuses that ordered pipeline behind its own Plan 9 profile and scheduling seam, and additionally moves large non-checkpointed files as bounded parallel segments of one temp because per-9P-handle throughput is capped (ADR 0052); same-spindle HDD batches reads and writes in bounded phases. Eligible remote streams may run independently. Values are chosen **statically from endpoint/device/topology profiles** (no adaptive tuning, no alternate OS-copy engines — VISION) | §5.8–§5.9, §8.2–§8.3 |
...
3. Query the same zero-access volume handle with `IOCTL_STORAGE_QUERY_PROPERTY`:
   ...
   - `MaximumTransferLength` is recorded as a device fact but no longer clamps the chosen chunk (the I/O manager splits buffered transfers into adapter-sized requests itself; ADR 0055); no BOT/UASP concurrency inference is claimed.
...
- **Locality:** plain-small parent directories normally hash to one worker, so NTFS/generic-UNC same-directory creates serialize while distinct directories proceed in parallel (a directory with ≥512 joined entries instead rotates its dispatch across 4 bounded affine lanes so one oversized directory cannot idle the pool — measured knee, ADR 0056). Any WSL endpoint — destination *or* source — stripes plain-small jobs across the round-robin shard instead, because the scarce operation on either side is the Plan 9 provider round trip, not an NTFS directory index (measured 2026-08-02: a 9P source pays ~2.3 ms of its 2.4 ms per-file cost in source round trips, so directory-affine serialization forfeited most of the pool; ADR 0052). A generic-redirector *source* makes the same switch only when the preflight volume queries — which the probe issues anyway — measured a network-class round trip (≥250 µs floor): loopback SMB measured faster under affinity (~2 µs round trips; the local create convoy outweighs them), while at LAN latencies the Plan 9 arithmetic applies unchanged. The sample is a static preflight decision under VISION's minor-measurement allowance, logged with the chosen locality, never re-tuned mid-run (ADR 0053). Redirector-streamed files use the same independent shard. Every queue remains bounded.
- **Interleaving:** under the standard transport the coordinator copies one large file/stream at a time while already-dispatched small jobs may remain in flight. Redirectors may run independent non-checkpointed streams on the worker pool while sparse/checkpointed work stays coordinator-owned. Under same-spindle HDD, the coordinator first drains the single phased worker so large/transactional/reparse destination I/O cannot interleave with its source/destination phases. There is no runtime throttle or user-controlled parallel-stream/queue-depth scheduler.
...
There is no finalizer/closer stage. Each worker owns the entire file, which measurement showed is already enough to overlap destination close latency. The hot-path rules are: no coordinator stream probe, one final-name destination open, one whole-buffer unnamed write, no hash unless verification requests it, and no extra metadata update when create-time stamping is safe. Anything added requires isolated correctness evidence; any speed claim additionally requires an approved isolated benchmark.

**Local NTFS relative-create specialization (ADR 0048, widened by ADR 0056):** when both endpoints are local NTFS and the static transport is standard (the former distinct-extents requirement was scoping conservatism over a destination-side mechanism; same-spindle pairs remain excluded through their transport kind), each directory-affine worker caches one verified destination-parent handle. Plain-small final names are opened with `NtCreateFile`, that handle as `RootDirectory`, and only the final UTF-16 component, avoiding repeated absolute-parent resolution. The parent denies delete sharing to pin that directory namespace; its kind and captured identity are checked once per worker/directory. An unavailable capability falls back to the ordinary absolute-path constructor. `FILE_CREATE` retains `CREATE_NEW` collision safety, while replacements retain same-handle snapshot validation before truncation. Inline, transactional, sparse, large, same-spindle, ReFS, FAT/exFAT, UNC, and WSL paths do not carry this capability. Its correctness and isolation are confined-test proven; the speedup remains pending the approved multi-drive NTFS benchmark in `BENCHMARKS.md`.

**WSL direct-create specialization (ADR 0046):** `CREATE_NEW` already proves a
new destination handle is the regular file just created, so new files skip a
redundant handle-metadata query. WSL destination handles receive the sequential
cache hint and defer projected last-write time until the mandatory post-write
stamp instead of issuing an initial Plan 9 metadata call that writes may
supersede. Replacement identity validation, post-read source validation, final
stamping, close-before-success, and rerun repair remain unchanged.
...
WSL owns a distinct audit/profile transport identity while reusing the
correctness-tested two-buffer redirector mechanics. That seam lets its request
size, worker routing, destination create hints, and future provider-specific
work evolve without altering generic SMB or either local transport (ADR 0046).

The throughput core, deliberately shaped by the complexity rule: one **stream** = one ordered file stream copied through synchronous buffered handles. Local standard's unnamed large streams use the same two-buffer overlap pipeline as the redirectors (ADR 0055 — half-duplex read-then-write alternation measured ~25% behind on distinct NVMe, falsifying the earlier assumption that cache-manager read-ahead/write-behind supplied the overlap); its sparse ranges and named streams keep the original sequential chunk loop. Generic redirector and WSL transports (ADRs 0045/0046) use exactly two request buffers and one scoped reader: the next source read may overlap the current destination write, while hashing and writes remain strictly ordered and every segment ends at a checkpoint boundary. Same-spindle HDD instead fills a bounded staging buffer through request-sized reads before draining it through request-sized writes; it never issues source and destination I/O concurrently. The IOCP overlapped ring and unbuffered reader/writer design remain deleted (ADR 0028): the fixed redirector pipeline has no completion port, unbuffered/alignment path, or depth knob. Structure:

- Source and destination are plain synchronous handles (destination created via the temp-name protocol); request chunks come from the class profile (§8.2) — the adapter MTL is a reported fact, never a clamp (ADR 0055). Redirectors fallibly allocate exactly two chunks per active stream. A same-spindle staging buffer defaults to 256 MiB, is fallibly allocated, never exceeds the remaining stream, and is capped by `--tune mem`; `--tune same-spindle-burst=` provides a bounded 1 MiB–1 GiB expert override. **No sector-alignment machinery exists anywhere** — buffered I/O needs none.
...
- There is one ordered streaming implementation shared by coordinator and redirector workers. Sparse or checkpoint-eligible streams remain coordinator-owned; eligible independent redirector streams may occupy separate bounded workers. The removed `streams` knob never affected execution and remains absent (ADR 0033). **There is no runtime governor** (F29): static chunks/workers/thresholds are chosen once, and a misbehaving bridge is the device breaker's job (§5.13).
...
- Plain small files and sub-threshold transactional ADS/EA files hash only with `--verify`; the default common path spends zero CPU on it (and per VISION there is no separate hash-recording option). **Large/checkpoint-capable streams always use xxh3** — expected cost well under 2 % of one core per GB/s (hypothesis H3, §8.7), required for checkpoint integrity (F12, §5.12), and lands a full-file digest in the log/journal at zero extra I/O. All hashes are computed from buffers the engine already holds (no extra reads), in **file-offset order** (§5.9); on the local standard pipeline the digest runs on its own stage between reader and writer so this cost stays off both hot legs (measured 2026-08-02: serialized into the write leg it held the pipeline at the alternation ceiling — ADR 0057).
...
Journal hygiene: append-only JSONL; every record CRC-tagged. Replay uses one-record lookahead rather than materializing journal history. An invalid/torn final record is truncated to the last complete record; an invalid interior record is ignored without being trusted or deleting later valid records, and an unsupported version fails closed without rewriting the file. Journal append failure at runtime → checkpointing disabled for the run (copies continue; large files lose resume; warning + audit note per §5.15). **Temp lifecycle (§4.3):** in-flight temps carry a pending delete disposition, so ordinary process kills leave none; resumable partials (≥ checkpoint threshold, disposition cleared at first checkpoint) are described by their checkpoint records. Invalid checkpoint candidates are discarded only through their identity-verified open handle. Power-loss or named-stream-window stragglers that are not live checkpoint candidates are reported as extras with a cleanup hint, never auto-deleted by path (I2/ADR 0027). `--fresh` ignores the journal and restarts under the same temp-safety rules.
...
**Audit-record safety** (the log and report *are* bigcp's claims, so they get guarantees of their own): the report is written to an exact-handle-owned sibling → synchronized → renamed by that same handle → synchronized/closed; `run_end` is appended and flushed to the log before the summary prints and the process exits.

**Audit-failure policy (F7/I7 coherence):** the machine-readable log is not optional decoration — F7 requires it and I7 promises every normally reported failure lands in it, so a run that can no longer log must stop making claims. On a log-write failure: (1) one reopen attempt on the same path; (2) failover of the log stream to the state directory (a different device in the common case), noted in the log itself and best-effort stderr. Only if **both** fail, abort the run immediately with exit 6. The deliberately deleted modeled audit-drain state (ADR 0027, §13.2) means this unrecoverable case promises neither a normal final report nor `run_end`; already committed files remain valid, an operation already executing may finish while worker resources unwind, and the next run reclassifies the destination authoritatively. Lesser degradations that lose no per-file events (e.g., journal failure → checkpointing disabled, §5.12; report unwritable → state-dir fallback path printed) mark `audit:"degraded"` and continue. "Infallible by buffering" is not a property this design claims anywhere.
...
select_profile(side):                       # once per volume at startup; never changes mid-run
  ...
  transport = same_spindle if overlap and either effective class is HDD
              else wsl if either endpoint is WSL
              else redirector if either endpoint is remote
              else standard
  if same_spindle: workers = 1; burst = bounded static default/override (§8.3)
  else if redirector and mem is set: reserve two coordinator chunks;
       cap workers by remaining / max(large_threshold, 2 * chunk)
  else if mem is set: reserve three pipelined coordinator chunks (ADRs 0055/0057); cap workers by remaining / large_threshold
...
| I7 | Every normally reported failure is logged with path + code + operation; a run that can no longer log stops making claims | `Outcome::Failed` carries all three by construction; audit-failure policy §5.15 (reopen → failover → immediate exit 6, with no terminal-record promise) |
...
| I9 | Copy-data buffers, journal replay, and worker queues are bounded; directory-join metadata is the documented pre-1.0 exception | bounded channels, one-record journal lookahead, fallible small/pipeline/burst reservation, exact standard `3×chunk + workers×threshold` (ADR 0057) and redirector `2×chunk + workers×max(threshold, 2×chunk)` accounting, serialized same-spindle burst cap; single-directory map gap tracked in §5.6/LIMITATIONS.md |
...
| Device/endpoint class (per side) | Chunk | Workers |
|---|---|---|
| NVMe (internal) | 16 MiB (measured 2026-08-02: beat 8 MiB by ~12–40% through the overlap pipeline) | `min(64, 4×cores)` |
| SATA SSD | 8 MiB | 32 |
| USB SSD | 4 MiB | 16 |
...
There are no stream-count, queue-depth, or enumeration-thread columns because the implementation has no such user-controlled schedulers (ADR 0033). **Composition is deterministic:** `chunk = min(src.chunk, dst.chunk, optional memory cap)` — the adapter MaximumTransferLength deliberately does not clamp it (it bounds one storport request; the I/O manager splits buffered transfers itself, and clamping to a VMD's 2 MiB MTL halved measured distinct-NVMe throughput — ADR 0055); worker count follows the destination row unless the source is HDD **or remote**, in which case the source row caps it. Explicit `--profile` can replace a remote Auto class row but cannot manufacture physical topology. Local intersecting rotational extents select one phased worker and a 256 MiB burst (§8.3). Generic UNC selects `redirector`; any WSL side selects the distinct `wsl` transport. Standard, redirector, and WSL large streams all move through the bounded overlap pipeline — two buffers on redirector/WSL, three plus a dedicated in-flight-hash stage on local standard (ADRs 0045/0046/0055/0057) — and eligible non-checkpointed remote streams can use the composed worker count. With `mem`, every non-same-spindle transport reserves its pipeline's coordinator chunks (two redirector/WSL, three standard); redirector workers additionally reserve `max(threshold, 2×chunk)` each, standard workers one threshold-sized buffer each, and same-spindle uses its single serialized burst. Every endpoint, value, topology fact, transport kind, and burst size is logged/reported (ADRs 0040/0045/0046).
...
- Sequential is everything: 16 MiB preferred requests, full dense-file preallocation, and offset-ordered writes.
...
| E37 | log/journal/report device full or disconnected mid-run | audit-failure policy governs (§5.15): lossless fallbacks continue with `audit:degraded`; if **both** log paths fail, the run aborts immediately with exit 6 and no normal report/`run_end` promise (an already executing operation may finish while workers unwind); journal-only failure disables checkpointing (§5.12) |
...
| E50 | configured log/report/state/journal roles collide through one resolved path | rejected pre-flight before any destination creation or artifact overwrite (§5.12, ADR 0040) |
| E51 | native/redirector enumeration returns a rooted, multi-component, NUL-bearing, or cross-record child name | safe wrapper rejects the complete batch before core can join or traverse the name (ADR 0040) |
...
Exit codes: 0 ok · 2 completed-with-failures · 3 user-canceled (resumable)
            ...
            (CLI usage/parse errors exit 5 — never 2, which is reserved for real completed-with-failures;
             unrecoverable audit failure follows §5.15)
```
...
- **Confinement:** every filesystem-mutating test operates exclusively under a newly created unique directory beneath a designated scratch root (or inside a dedicated test VHDX stored there). Integration/testkit paths go through `SandboxRoot`, which **refuses** absolute or escaping children. Crate-local Win32 wrapper tests cannot depend back on `testkit` without creating a dependency cycle, so they use an owned `tempfile::TempDir` and derive every path from it; CI redirects `TEMP`/`TMP` into a unique C: run root before tests start. A CI lint rejects dangerous storage/machine commands and asserts that the sandbox whitelist, entry caps, and end-to-end root guard remain present. The oracle's containment check (§12.2) protects integration runs — nothing outside the intended destination may change.
- **Enforcement is structural, not procedural** — this matters doubly when tests are written and executed by AI agents working autonomously: the testkit sandbox's root/child refusal, crate-local ownership of one unique temporary directory, CI-controlled `TEMP`/`TMP`, and the source safety lint are the binding controls; the §14.3 checklist re-affirmation is a backstop. Tests that mount volumes may only mount VHDX files living inside the sandbox — never real volumes, never raw physical devices, no diskpart/format against real drives, ever. An implementing agent must treat these rules as hard constraints that no task instruction overrides.
...
The semantic contract (§4), direct-plain-small and transactional auxiliary/sparse/large completion strategies in one product engine (ADR 0043), isolated standard/generic-redirector/WSL/same-spindle transports (ADRs 0036/0045/0046), isolated UNC/WSL endpoint policy (ADR 0037), verified checkpoint resume, exact-handle-owned atomic state/report publication (ADRs 0038/0041), fail-closed native and sandbox boundaries (ADR 0039), structural artifact/resource bounds (ADR 0040), both verification forms, the independent oracle, JSONL log + report, exact classification, the CLI grammar, the device/space/redirector circuit breaker (exit 4), mid-file graceful cancellation, and the ETA display are **built and routinely tested** within the §12.0 sandbox. Non-NTFS endpoints remain explicitly best-effort and outside the release-certification gate (ADR 0042).
...
What remains before a 1.0 claim is primarily verification/evidence for the NTFS contract, plus one product-scaling gap: a bounded single-directory enumeration fallback (§5.6). The on-request validation pass is §12.10; optional performance candidates live in BENCHMARKS.md; disposition history is in `docs/REVIEW_2026-07-29.md` and the ADRs from 0027 onward. **Deviation rule:** any future intentional difference from this plan is recorded before the deviating code merges — either as a normative edit here or an ADR — there is no separate deviations file. Open gate highlights:
...
Several scope extensions may return later: UDF/third-party local filesystems, protocol-specific server-side acceleration, and native Linux/macOS ports. FAT/exFAT, efficient same-spindle copy, and UNC/WSL interoperability plus bounded client-side redirector overlap have landed through the seams previously described here (ADRs 0035/0036/0037/0045/0046):
...
- **Network/UNC/WSL (implemented):** `endpoint.rs` owns local/generic-UNC/WSL classification; `path.rs` owns extended syntax and WSL alias canonicalization; `volume.rs` alone owns handle-bound remote filesystem queries; `device.rs` alone blocks remote local-device IOCTLs; `FilesystemPolicy` and `devprofile.rs` own remote projection/name/preallocation/profile choices. `transport.rs` owns distinct generic-redirector and WSL identities over the shared two-buffer overlap, `worker.rs` owns eligible-stream concurrency plus either-side WSL (and latency-gated remote-source) striping, `engine.rs` owns the WSL segmented-transfer planner (`segment_plan`/`copy_streamed_segmented`, ADR 0052), and `file.rs` owns WSL sequential hints, deferred stamping, and the identity-proven `SegmentWriter`. The shared engine retains ordered hashing/checkpoints/publication. Protocol-specific server-side acceleration belongs behind this seam if evidence later justifies it.
...
| Remote provider latency, disconnect, or misleading optional capabilities | lower throughput, interrupted handles, projected fidelity | isolated bounded UNC/WSL profiles, two-buffer ordered overlap plus eligible-file concurrency, WSL-specific create striping/round-trip reduction, server-advertised capability gates, one startup acceptance, redirector-loss breaker, abort-and-rerun, projected verification (ADRs 0037/0045/0046) |
...
| Streaming I/O | synchronous buffered `ReadFile`/`WriteFile` in profile-sized chunks: request-at-a-time for standard sparse ranges and named streams, bounded overlap for standard unnamed large streams (three buffers plus an in-flight-hash stage — ADRs 0055/0057), fixed two-buffer source/destination overlap on generic redirectors and WSL, phased bursts on same-spindle; WSL destination handles add `FILE_FLAG_SEQUENTIAL_SCAN` as a cache hint (no `FILE_FLAG_NO_BUFFERING`, completion-port APIs, or alignment rules — §5.9, ADRs 0028/0036/0045/0046) |
...
- [MS-smb-copy] Microsoft SMB file-transfer performance guidance — https://learn.microsoft.com/en-us/troubleshoot/windows-server/networking/slow-smb-file-transfer
- [MS-smb-credits] SMB2 multi-credit request rules — https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-smb2/0edce9a0-766e-41aa-a3b7-2044defbbb8f
...
| F37 UNC/mapped/WSL support without local regression | §4.4–§4.5, §5.5, §5.9, §8.2, §10.1 | endpoint/path/policy/profile/error units; distinct WSL audit/profile selection; bounded pipeline ordering/overlap/cancel/memory/dispatch units; WSL destination striping and deferred-stamp tests; E44/E47–E49; optional approved remote compatibility/performance cells (§12.5, ADRs 0042/0045/0046) |
