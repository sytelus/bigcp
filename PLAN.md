...
**Two completion protocols by size (ADR 0030).** *Small files* (< large threshold, every stream) write directly to the final name: create (`CREATE_NEW`, or truncate-in-place for replacements — preserving the destination's security descriptor and clearing a known read-only attribute first), write every stream, write EAs, then **stamp source timestamps strictly last** — the completion marker. An interrupted partial therefore carries a non-matching size or mtime and the next run replaces it; that window is the accepted price of halving per-file filter work (VISION's rerun contract). *Large files* keep the temp protocol below, because a checkpointed resume must verify a temp identity and the rename cost is amortized to nothing:
...
| I5 | **Run-level convergence (rescoped by ADR 0030):** a kill at any instant leaves a destination a re-run provably converges to correctness from. Small files write their final name directly with timestamps stamped strictly last, so an interrupted partial can never match the exact size+mtime heuristic and is replaced on rerun; large files still publish only via temp+rename, so no *large* final name is ever incomplete | mtime-last is structural in `DestinationFinal::finish`; chaos suite asserts kill-anywhere → rerun converges (oracle-verified) |
