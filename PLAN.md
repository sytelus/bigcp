...
| I9 | Copy-data buffers, journal replay, and worker queues are bounded; directory-join metadata is the documented pre-1.0 exception | bounded channels, one-record journal lookahead, fallible small/burst-buffer reservation, exact standard-path `chunk + workers×threshold` memory accounting, serialized same-spindle burst cap; single-directory map gap tracked in §5.6/LIMITATIONS.md |
...
There are no stream-count, queue-depth, or enumeration-thread columns because the implementation has no such schedulers (ADR 0033). **Composition is deterministic:** `chunk = min(src.chunk, dst.chunk, both nonzero local MTLs, optional memory cap)`; worker count follows the destination row unless the source is HDD **or remote**, in which case the source's conservative row caps it. Explicit `--profile` can replace a remote Auto class row but cannot manufacture physical topology. If local disk-number extents intersect and either effective class is HDD, §8.3 replaces that composed worker count with one phased worker and selects a 256 MiB burst (capped by memory; bounded expert override available). On the standard path, `mem` reserves the coordinator chunk that can coexist with worker activity, then caps threshold-sized workers with the remainder; it must hold at least one of each. Same-spindle coordinator work drains the phased worker first, so only one capped burst is live and no fictitious second chunk is reserved. Every selected endpoint, value, topology fact, transport kind, and burst size is logged/reported (ADR 0040).
