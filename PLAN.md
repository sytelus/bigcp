...
| I/O strategy | **One product engine, two completion paths and three isolated transports**: local standard retains request-at-a-time buffered streaming; redirectors overlap one read with one write through two bounded buffers and may parallelize independent non-checkpointed streams; same-spindle HDD batches reads and writes in bounded phases. Values are chosen **statically from endpoint/device/topology profiles** (no adaptive tuning, no alternate OS-copy engines — VISION) | §5.8–§5.9, §8.2–§8.3 |
...
- **Locality:** plain-small parent directories hash to one worker, so same-directory creates serialize while distinct directories proceed in parallel. Redirector-streamed files use a separate round-robin shard so the first independent large files in one directory occupy distinct workers. Every queue remains bounded.
...
There are no stream-count, queue-depth, or enumeration-thread columns because the implementation has no such user-controlled schedulers (ADR 0033). **Composition is deterministic:** `chunk = min(src.chunk, dst.chunk, both nonzero local MTLs, optional memory cap)`; worker count follows the destination row unless the source is HDD **or remote**, in which case the source row caps it. Explicit `--profile` can replace a remote Auto class row but cannot manufacture physical topology. Local intersecting rotational extents select one phased worker and a 256 MiB burst (§8.3). Any remote endpoint selects the fixed two-buffer redirector transport; eligible non-checkpointed streams can use the composed worker count (ADR 0045). With `mem`, standard reserves one coordinator chunk and threshold-sized workers, redirector reserves two coordinator chunks and `max(threshold, 2×chunk)` per worker, and same-spindle uses its single serialized burst. Every endpoint, value, topology fact, transport kind, and burst size is logged/reported (ADRs 0040/0045).
...
The semantic contract (§4), direct-plain-small and transactional auxiliary/sparse/large completion strategies in one product engine (ADR 0043), isolated standard/redirector/same-spindle transports (ADRs 0036/0045), isolated UNC/WSL endpoint policy (ADR 0037), verified checkpoint resume, exact-handle-owned atomic state/report publication (ADRs 0038/0041), fail-closed native and sandbox boundaries (ADR 0039), structural artifact/resource bounds (ADR 0040), both verification forms, the independent oracle, JSONL log + report, exact classification, the CLI grammar, the device/space/redirector circuit breaker (exit 4), mid-file graceful cancellation, and the ETA display are **built and routinely tested** within the §12.0 sandbox. Non-NTFS endpoints remain explicitly best-effort and outside the release-certification gate (ADR 0042).
...
- **Network/UNC (implemented):** `endpoint.rs` owns local/generic-UNC/WSL classification; `path.rs` owns extended syntax and WSL alias canonicalization; `volume.rs` alone owns handle-bound remote filesystem queries; `device.rs` alone blocks remote local-device IOCTLs; `FilesystemPolicy` and `devprofile.rs` own remote projection/name/preallocation/profile choices. `transport.rs` owns two-buffer read/write overlap, `worker.rs` owns eligible-stream concurrency/cancellation, and the shared engine retains ordered hashing/checkpoints/publication. Protocol-specific server-side acceleration belongs behind this seam if evidence later justifies it.
...
| Remote provider latency, disconnect, or misleading optional capabilities | lower throughput, interrupted handles, projected fidelity | isolated bounded UNC/WSL profiles, two-buffer ordered overlap plus eligible-file concurrency, server-advertised capability gates, one startup acceptance, redirector-loss breaker, abort-and-rerun, projected verification (ADRs 0037/0045) |
...
| Streaming I/O | synchronous buffered `ReadFile`/`WriteFile` in profile-sized chunks: sequential on standard, fixed two-buffer source/destination overlap on redirectors, phased bursts on same-spindle (no `FILE_FLAG_NO_BUFFERING`, completion-port APIs, or alignment rules — §5.9, ADRs 0028/0036/0045) |
...
| F37 UNC/mapped/WSL support without local regression | §4.4–§4.5, §5.5, §5.9, §8.2, §10.1 | endpoint/path/policy/profile/error units; bounded pipeline ordering/overlap/cancel/memory/dispatch units; E44/E47–E49; optional approved remote compatibility/performance cells (§12.5, ADRs 0042/0045) |
