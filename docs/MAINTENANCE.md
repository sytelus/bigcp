...
| I9 | Memory/work queues are bounded. | `crossbeam_channel::bounded`; every non-same-spindle transport reserves its pipeline's coordinator chunks (two on redirector/WSL, three on local standard — ADRs 0055/0057), redirector workers additionally reserve two chunks per active stream, same-spindle coordinator/worker activity is serialized under one burst cap with a compile-time-guarded 4096-file batch handle bound (ADR 0056), and the worker pool's result ring materializes lazily on the first job (ADR 0057). |
